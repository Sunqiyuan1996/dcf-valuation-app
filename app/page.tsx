      ...
      <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-6 py-4">
        <div>
          <h3 className="text-sm font-semibold">
            {title}
            {chapter && <span className="ml-2 text-[11px] font-normal uppercase tracking-wide text-slate-400">{chapter}</span>}
          </h3>
...
function Headline({ label, value, accent, tone }: { label: string; value: string; accent?: boolean; tone?: 'good' | 'bad' }) {
  const toneClass = tone === 'good' ? 'text-positive' : tone === 'bad' ? 'text-negative' : accent ? 'text-accent' : 'text-ink';
  return (
    <div className="px-6 py-5">
      <div className="text-[11px] uppercase tracking-wide text-slate-400">{label}</div>
      <div className={`mt-1 text-3xl font-semibold tracking-tight tabular-nums ${toneClass}`}>{value}</div>
    </div>
  );
}
...
function Tile({ label, value, sub, tone }: { label: string; value: string; sub: string; tone?: 'good' | 'bad' | 'warn' }) {
  const border =
    tone === 'good' ? 'border-l-positive' : tone === 'bad' ? 'border-l-negative' : tone === 'warn' ? 'border-l-warn' : 'border-l-slate-300';
  return (
    <div className={`rounded-xl border border-slate-200 border-l-4 bg-white px-4 py-3 shadow-sm ${border}`}>
      <div className="text-[11px] uppercase tracking-wide text-slate-400">{label}</div>
